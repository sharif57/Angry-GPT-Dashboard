...
const DashboardHomeTable = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedTransaction, setSelectedTransaction] = useState(null);
  const { data: transactionsData } = useAllTransactionGetQuery(); // Assuming this hook fetches the transaction data
  ...
  return (
    <div className="rounded-lg border py-4 border-black mt-8 recent-users-table">
      <h3 className="text-2xl text-white mb-4 pl-2">Recent Transactions</h3>
      <Table
        columns={columns}
      ...
      >
        {selectedTransaction && (
          <div>
            ...
            <p>
              <strong>Order ID:</strong> {selectedTransaction.order}
            </p>
          </div>
        )}
      </Modal>
